...
class CustomerController extends Controller
{
    ...
    public function forgot_verify(Request $request)
    {
        ...
        Toastr::success('Success','Your forgot OTP send successfully');
    ...
    public function order_save(Request $request)
    {
        ...
        foreach ($this->seller_order() as $seller_id) {
            ...
            $order->amount              = ($subtotal + $additional_shipping + $shippingfee) - $discount;
        ...
        if ($sms_gateway) {
            ...
            $data = [
                ...
                "contacts" => '01911219721',
                "type" => 'text',
                "senderid" => "$sms_gateway->serderid",
                "msg" => "Dear $request->name!\r\nYour order ($order->invoice_id) has been successfully placed. Track your order https://upxar.com/customer/order-track and Total Bill $order->amount\r\nThank you for using $site_setting->name"
            ];
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_POST, 1);
