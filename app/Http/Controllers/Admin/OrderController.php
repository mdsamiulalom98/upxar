...
class OrderController extends Controller
{
    ...
    public function order_status(Request $request)
    {
        ...
        if ($request->order_status == 2 || $request->order_status == 7) {
            $orders = Order::whereIn('id', $request->input('order_ids'))->get();
                ...
                if($sms_gateway) {
                    ...
                    $data = [
                        ...
                        "msg" => "Dear $name!\r\nYour order ($order->invoice_id) has been  $status. Track your order https://upxar.com/customer/order-track and Total Bill $order->amount Tk\r\nThank you for using $site_setting->name"
                    ];
                    $ch = curl_init();
            ...
            foreach ($orders as $order) {
                ...
                foreach ($orders_details as $order_detail) {
                    if ($order_detail->product_type == 1) {
                        ...
                        $product = ProductVariable::where(['product_id' => $order_detail->product_id, 'size' => $order_detail->product_size])->first();
                        $product->stock -= $order_detail->qty;
                        $product->save();
                    }
                }
